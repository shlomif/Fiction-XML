...
foreach my $fn (@tests)
{
...

    {
        ...
        # TEST*$num_texts
        is( scalar(@title), 1, "Found one global <db:title>", );
    ...
    foreach
        my $xpath ( q{//x:html/x:head/x:title}, q{//x:html/x:body/x:div/x:h1}, )
    {
        my @title = $xhtml_find->($xpath);

        # TEST*$n
        is( scalar(@title), 1, "Found one global <x:title>", );
    ...
    {
        {
            ...
            is( scalar(@elems), 1, "Found bold tag", );

            # TEST*$num_with_styles
            like( $elems[0]->toString(),
                qr{swear}, "Elem[0] is the right <b> tag." );

            @elems = $xhtml_find->(q{//x:div/x:p/x:i});

            # TEST*$num_with_styles
            is( scalar(@elems), 1, "Found italic tag", );

            # TEST*$num_with_styles
            like( $elems[0]->toString(),
                qr{David}, "<i>[0] contains the right contents." );
        }

        {
            ...
            @elems = $db_find->(
                q{//db:article/db:section/db:para/db:emphasis[@role="bold"]});

            # TEST*$num_with_styles
            is( scalar(@elems), 1, "DocBook: found bold tag", );

            # TEST*$num_with_styles
            is( $elems[0]->textContent(),
                "swear", "Elem[0] is the right <emphasis role=bold> tag." );
            ...
            is(
                $elems[0]->textContent(),
                "David", "<i>[0] contains the right contents.",
            );
        }
        ...
        # Test the DocBook/XML incorporation of the <title> tag.
        {
            ...
            @elems =
                $db_find->(q{//db:section[@xml:id='goliath']/db:info/db:title});

            # TEST*$num_with_styles
            is( scalar(@elems), 1, "DocBook: found one title tag", );
